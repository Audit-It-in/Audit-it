...
import { MapPinIcon, ChatCircleIcon, SparkleIcon } from "@phosphor-icons/react";
...
export const ProfileHeader: React.FC<ProfileHeaderProps> = ({
  ...
  return (
    <Card variant='default' size='default' className='rounded-2xl border-2 border-primary-100/60 shadow-neumorphic-xl'>
      <div className='grid grid-cols-12 gap-6 items-start'>
          ...
          <div className='space-y-2.5 flex-1'>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              ...
              {languages.slice(0, 3).map((lang, i) => (
                ...
                >
                  <span className='text-[11px] font-semibold text-neutral-700'>{lang}</span>
                </div>
              ))}
              {languages.length > 3 && (
                <div className='px-2.5 py-1.5 rounded-xl shadow-neumorphic-inset bg-neutral-50 border border-neutral-200/60'>
                  <span className='text-[11px] font-semibold text-neutral-700'>+{languages.length - 3}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className='col-span-12 lg:col-span-5 flex flex-col gap-3 min-h-[180px]'>
          <h3 className='text-xs font-semibold text-primary-700 uppercase tracking-wide text-center'>
            Specializations
          </h3>
          {displaySpecializations.length > 0 && (
            <div className='rounded-2xl shadow-neumorphic-inset-deep bg-neutral-50 border-2 border-primary-100/60 p-3'>
              <div className='flex gap-2 overflow-x-auto snap-x sm:grid sm:grid-cols-2 sm:gap-2'>
                ...
                {specializations.length > displaySpecializations.length && (
                  <button
                    ...
                    className='flex items-center justify-center px-2.5 py-1.5 rounded-xl shadow-neumorphic-md bg-white border-2 border-neutral-200/70 hover:border-neutral-300/80 transition-colors'
                  >
